...
object MethodConfigResolver {
  ...
  def resolveInputs(workspaceContext: WorkspaceContext, methodConfig: MethodConfiguration, entity: Entity, wdl: String): Map[String, SubmissionValidationValue] = {
    NamespaceWithWorkflow.load(wdl).workflow.inputs map { case (fqn: FullyQualifiedName, wfInput: WorkflowInput) =>
      val result = methodConfig.inputs.get(fqn) match {
        case Some(AttributeString(expression)) =>
          evaluateResult(workspaceContext, entity, expression) match {
            ...
            case Failure(regret) => SubmissionValidationValue(None, Some(regret.getMessage), fqn)
          }
        case _ =>
          ...
          SubmissionValidationValue(None, errorOption, fqn)
        }
        (fqn, result)
      } toMap
    }
