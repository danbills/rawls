...
class HttpExecutionServiceDAO( executionServiceURL: String, submissionTimeout: FiniteDuration )( implicit val system: ActorSystem ) extends ExecutionServiceDAO with DsdeHttpDAO with Retry with FutureSupport with LazyLogging {
  ...
  val logRequest: HttpRequest => HttpRequest = { req => logger.info(s"%s rq %s".format(this, req.toString())); req}
  val logResponse: HttpRequest => HttpRequest = { resp => logger.info(s"%s resp %s".format(this, resp.toString())); resp}
  ...
  override def submitWorkflows(wdl: String, inputs: Seq[String], options: Option[String], userInfo: UserInfo): Future[Seq[Either[ExecutionServiceStatus, ExecutionServiceFailure]]] = {
    ...
    val pipeline = addAuthHeader(userInfo) ~> logRequest ~> sendReceive ~> logResponse ~> unmarshal[Seq[Either[ExecutionServiceStatus, ExecutionServiceFailure]]]
  ...
  override def status(id: String, userInfo: UserInfo): Future[ExecutionServiceStatus] = {
    ...
    import system.dispatcher
    val pipeline = addAuthHeader(userInfo) ~> logRequest ~> sendReceive ~> logResponse ~> unmarshal[ExecutionServiceStatus]
  ...
  override def callLevelMetadata(id: String, userInfo: UserInfo): Future[ExecutionMetadata] = {
    ...
    import system.dispatcher
    val pipeline = addAuthHeader(userInfo) ~> logRequest ~> sendReceive ~> logResponse ~> unmarshal[ExecutionMetadata]
    retry(when500) { () => pipeline(Get(url)) }
  }
  ...
  override def outputs(id: String, userInfo: UserInfo): Future[ExecutionServiceOutputs] = {
    ...
    import system.dispatcher
    val pipeline = addAuthHeader(userInfo) ~> logRequest ~> sendReceive ~> logResponse ~> unmarshal[ExecutionServiceOutputs]
    retry(when500) { () => pipeline(Get(url)) }
  }
  ...
  override def logs(id: String, userInfo: UserInfo): Future[ExecutionServiceLogs] = {
    ...
    import system.dispatcher
    val pipeline = addAuthHeader(userInfo) ~> logRequest ~> sendReceive ~> logResponse ~> unmarshal[ExecutionServiceLogs]
    retry(when500) { () => pipeline(Get(url)) }
  }
