  ...
  it should "get a workspace" in withTestWorkspacesApiServices { services =>
    Get(s"/workspaces/${testWorkspaces.workspaceOwner.namespace}/${testWorkspaces.workspaceOwner.name}") ~>
      ...
      check {
        ...
        services.dataSource.inTransaction { txn =>
          assertResult(
            WorkspaceListResponse(WorkspaceAccessLevel.Owner, testWorkspaces.workspaceOwner, WorkspaceSubmissionStats(None, None, 0), Await.result(MockGoogleCloudStorageDAO.getOwners(testWorkspaces.workspaceOwner.workspaceId), Duration.Inf))
          ){
  ...
  it should "list workspaces" in withTestWorkspacesApiServices { services =>     Get("/workspaces") ~>
      ...
      check {
        ...
        services.dataSource.inTransaction { txn =>
          assertResult(Set(
            WorkspaceListResponse(WorkspaceAccessLevel.Owner, testWorkspaces.workspaceOwner, WorkspaceSubmissionStats(None, None, 0), Await.result(MockGoogleCloudStorageDAO.getOwners(testWorkspaces.workspaceOwner.workspaceId), Duration.Inf)),
            WorkspaceListResponse(WorkspaceAccessLevel.Write, testWorkspaces.workspaceWriter, WorkspaceSubmissionStats(Option(testDate), Option(testDate), 2), Await.result(MockGoogleCloudStorageDAO.getOwners(testWorkspaces.workspaceOwner.workspaceId), Duration.Inf)),
            WorkspaceListResponse(WorkspaceAccessLevel.Read, testWorkspaces.workspaceReader, WorkspaceSubmissionStats(None, None, 0), Await.result(MockGoogleCloudStorageDAO.getOwners(testWorkspaces.workspaceOwner.workspaceId), Duration.Inf))
          )) {
